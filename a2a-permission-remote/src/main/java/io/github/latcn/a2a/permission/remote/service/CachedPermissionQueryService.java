package io.github.latcn.a2a.permission.remote.service;
...
public class CachedPermissionQueryService implements PermissionQueryService {
    ...
    public CachedPermissionQueryService(
        ...
        this.remotePermissionQueryService = remotePermissionQueryService;
    ...
    public UserFullPermissionDTO getUserFullPermissions(Long userId) {
        if (!cacheConfig.isEnableLocalCache()) {
            log.debug("Local cache disabled, fetching user permissions from remote service for userId: {}", userId);
        ...
        try {
            ...
            if (result != null && isCacheValid(result)) {
                log.debug("Cache hit for user permissions, userId: {}", userId);
                return result;
            }
            
            log.debug("Cache invalid for userId: {}, refreshing...", userId);
    ...
    public RemotePermissionQueryService getRemotePermissionQueryService() {
        return remotePermissionQueryService;
    }
}